...
t.test("check detects powershell on Windows", function()
  t.reset("yankdown.check")
  with_env({ has = { win32 = 1 } }, { pandoc = true, ["powershell.exe"] = true }, function()
    local deps = require("yankdown.check").check()
    local d = vim.iter(deps):find(function(dep)
      return dep.name == "powershell.exe"
    end)
    t.eq(d.found, true)
    t.eq(d.optional, false)
...
t.test("check detects missing powershell on Windows", function()
  t.reset("yankdown.check")
  with_env({ has = { win64 = 1 } }, { pandoc = true, ["powershell.exe"] = false }, function()
    local deps = require("yankdown.check").check()
    local d = vim.iter(deps):find(function(dep)
      return dep.name == "powershell.exe"
    end)
    ...
    t.eq(d.optional, false)
    t.eq(d.platform, "Windows")
    t.eq(require("yankdown.check").ok(deps), false)
  end)
end)
